...
function onNotificationClickActionHandling(clickAction) {
  // Xử lý hành động khi người dùng nhấn vào thông báo
  console.log('Handling notification click action:', clickAction);

  // Ví dụ: Chuyển hướng người dùng đến một màn hình cụ thể
  switch (clickAction) {
    case 'OPEN_URL':
      // Mở một URL cụ thể
      Linking.openURL('phoenixcampcrm://app');
      break;
    case 'VIEW_MESSAGE':
      // Hiển thị tin nhắn cụ thể
      // Code xử lý hiển thị tin nhắn ở đây
      break;
    default:
      // Xử lý hành động mặc định hoặc không hợp lệ
      console.log('Unknown or unsupported click action:', clickAction);
  }
}
